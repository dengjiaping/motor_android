...
public class AlbumActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.select_morephoto_album);
	...
	private void initSelectImage() {
		...
		for(final String path:selectedDataList){
			...
			ImageManager2.from(AlbumActivity.this).displayImage(imageView, path,R.drawable.default_icon,100,100);
		...
		okButton.setText("完成("+selectedDataList.size()+"/5)");
	}
	...
	private void initListener() {
		...
		gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
			...
			public void onItemClick(final ToggleButton toggleButton, int position, final String path,boolean isChecked) {
				...
				if(selectedDataList.size()>=5){
					toggleButton.setChecked(false);
					if(!removePath(path)){
						Toast.makeText(AlbumActivity.this, "只能选择5张图片", 200).show();
					}
					return;
				}
